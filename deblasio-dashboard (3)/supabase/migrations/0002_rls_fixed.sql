...
create or replace function public.app_agency_id_for_user()
returns uuid
language sql
security definer
...
create or replace function public.app_agency_role_for_user()
returns agency_role
language sql
security definer
set search_path = public
...
create or replace function public.app_client_ids_for_user()
returns uuid[]
language sql
security definer
set search_path = public
...
create or replace function public.app_assigned_client_ids_for_user()
returns uuid[]
language sql
security definer
set search_path = public
...
as $$
  select case
    when public.app_agency_role_for_user() = 'agency_admin' then
      (select coalesce(array_agg(id), '{}') from clients
       where agency_id = public.app_agency_id_for_user())
    else
...
create or replace function public.app_is_agency_user()
returns boolean language sql security definer set search_path = public stable as $$
  select exists (select 1 from agency_users where profile_id = auth.uid());
$$;

create or replace function public.app_is_client_user()
returns boolean language sql security definer set search_path = public stable as $$
...
create policy agencies_select on agencies for select
  using (id = public.app_agency_id_for_user());

create policy profiles_select_self_or_same_org on profiles for select
  using (
    id = auth.uid()
    or (public.app_is_agency_user() and exists (
          select 1 from agency_users au
          where au.profile_id = profiles.id
            and au.agency_id = public.app_agency_id_for_user()))
    or (public.app_is_agency_user() and exists (
          select 1 from client_users cu
          where cu.profile_id = profiles.id
            and cu.client_id = any(public.app_assigned_client_ids_for_user())))
  );
...
create policy agency_users_select on agency_users for select
  using (agency_id = public.app_agency_id_for_user());

create policy agency_users_write_admin_only on agency_users for all
  using (agency_id = public.app_agency_id_for_user() and public.app_agency_role_for_user() = 'agency_admin')
  with check (agency_id = public.app_agency_id_for_user() and public.app_agency_role_for_user() = 'agency_admin');
...
create policy clients_select_agency on clients for select
  using (
    (public.app_is_agency_user() and agency_id = public.app_agency_id_for_user()
      and id = any(public.app_assigned_client_ids_for_user()))
    or (public.app_is_client_user() and id = any(public.app_client_ids_for_user()))
  );

create policy clients_write_agency_staff on clients for insert
  with check (agency_id = public.app_agency_id_for_user() and public.app_agency_role_for_user() in ('agency_admin','account_manager'));

create policy clients_update_agency_staff on clients for update
  using (agency_id = public.app_agency_id_for_user()
    and (public.app_agency_role_for_user() in ('agency_admin','account_manager')
         and id = any(public.app_assigned_client_ids_for_user())));
...
create policy client_users_select on client_users for select
  using (
    profile_id = auth.uid()
    or (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()))
  );

create policy client_users_write_agency_admin on client_users for all
  using (public.app_agency_role_for_user() in ('agency_admin','account_manager')
    and exists (select 1 from clients c where c.id = client_users.client_id
                and c.agency_id = public.app_agency_id_for_user()))
...
create policy client_assignments_write_admin on client_assignments for all
  using (public.app_agency_role_for_user() = 'agency_admin')
  with check (public.app_agency_role_for_user() = 'agency_admin');
...
create policy client_services_select on client_services for select
  using (
    (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()))
    or (public.app_is_client_user() and client_id = any(public.app_client_ids_for_user()))
  );

create policy client_services_write_agency on client_services for all
  using (public.app_is_agency_user() and agency_role_for_user_ok())
  with check (agency_id = public.app_agency_id_for_user());
...
language sql security definer set search_path = public stable as $$
  select public.app_agency_role_for_user() in ('agency_admin','account_manager');
$$;
...
create policy campaigns_write on campaigns for all
  using (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()))
  with check (agency_id = public.app_agency_id_for_user());
...
create policy deliverables_select_client on deliverables for select
  using (public.app_is_client_user() and client_id = any(public.app_client_ids_for_user()) and client_visible = true);
create policy deliverables_write on deliverables for all
  using (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()))
  with check (agency_id = public.app_agency_id_for_user());

create policy tasks_select_agency on tasks for select
  using (public.app_is_agency_user() and (client_id is null or client_id = any(public.app_assigned_client_ids_for_user())));
...
create policy comments_select_client on comments for select
  using (public.app_is_client_user() and client_id = any(public.app_client_ids_for_user()) and visibility = 'client');
create policy comments_insert on comments for insert
  with check (
    (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()) and agency_id = public.app_agency_id_for_user())
    or (public.app_is_client_user() and client_id = any(public.app_client_ids_for_user()) and visibility = 'client')
  );
...
create policy approvals_select_client on approvals for select
  using (public.app_is_client_user() and client_id = any(public.app_client_ids_for_user()));
create policy approvals_write_agency on approvals for insert
  with check (public.app_is_agency_user() and client_id = any(public.app_assigned_client_ids_for_user()) and agency_id = public.app_agency_id_for_user());
